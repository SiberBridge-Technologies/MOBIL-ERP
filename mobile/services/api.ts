...
export async function apiRequest<T = any>(
  ...
  if (rawText) {
    try {
      ...
      // Sunucu geçerli JSON döndürmedi (örn. bir PHP hatası/uyarısı JSON'un
      // önüne eklenmiş olabilir). Sessizce yutmak yerine ham yanıtı göster.
      throw new ApiError(
        `Sunucudan geçersiz yanıt alındı: ${rawText.slice(0, 300)}`,
        response.status
      );
    }
  }
